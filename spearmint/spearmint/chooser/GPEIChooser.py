...
class GPEIChooser:
    ...
    def next(self, grid, values, durations, candidates, pending, complete):
        ...
        if self.mcmc_iters > 0:
            ...
            for i in range(self.mcmc_iters):
                ax.plot(cand[:, 0][sort_i], overall_ei[:, i][sort_i], '0.5', alpha=0.5)
            ...
            for i in range(self.mcmc_iters):
                ax.plot(cand[:, 0][sort_i], gp_mean[:, i][sort_i], '0.5', alpha=0.5)
    ...
    def compute_ei(self, comp, pend, cand, vals):
        if pend.shape[0] == 0:
            ...
            return np.mean(ei, axis=1), np.mean(func_m, axis=1)
